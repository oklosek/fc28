...
from backend.core.db import SessionLocal, VentState, RuntimeState
...
def get_state():
    with SessionLocal() as s:
        ...
        for v in controller.vents.values():
            vents.append(VentDTO(id=v.id, name=v.name, position=v.position,
                                 available=v.available, user_target=v.user_target))
        sensors = {
        ...
        return StateDTO(mode=controller.mode, vents=vents, sensors=sensors, config=CONTROL)
